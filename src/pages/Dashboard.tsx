...
export default function Dashboard() {
  ...
  const [failedReading, setFailedReading] = useState<any>(null);
  const [pendingIssueId, setPendingIssueId] = useState<any>(null);
  ...
  const addIssueAction = useMutation(api.compliance.addIssueAction);
  ...
  async function saveTemperature() {
    ...
    const result = await recordTemperature({ roundId, locationId: dashboard.location._id, equipmentId: currentEquipment._id, temperature: value });
    if (value > 8) {
      setFailedReading({ name: currentEquipment.name, temperature: value });
      setPendingIssueId(result.issueId);
      toast.error("Action required", { description: `${currentEquipment.name} recorded ${value}°C.` });
      return;
    }
    toast.success("Reading saved", { description: `${currentEquipment.name} · ${value}°C` });
  ...
  async function saveCorrectiveAction() {
    if (!pendingIssueId || !actionNotes.trim()) return;
    await addIssueAction({ issueId: pendingIssueId, action: `${actionSelections.join(", ")}${actionSelections.length ? ". " : ""}${actionNotes.trim()}` });
    setFailedReading(null); setPendingIssueId(null); setActionSelections([]); setActionNotes("");
    if (roundIndex + 1 >= activeEquipment.length && roundId && dashboard) {
      await completeRound({ roundId, locationId: dashboard.location._id });
      setShowRound(false); setRoundId(null); toast.success("AM round complete", { description: "All readings and corrective actions are recorded." });
    } else { setRoundIndex((index) => index + 1); setTemperature(""); }
  }
    ...
    <div className="min-h-screen bg-[#f6f7f5] text-[#171918]">
      <header className="sticky top-0 z-20 border-b border-black/[0.07] bg-[#f6f7f5]/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-8">
          <div className="flex items-center gap-3"><BrandMark /><div><p className="text-[15px] font-semibold tracking-tight">le Feast Daily Checks</p><p className="text-[11px] font-medium uppercase tracking-[0.16em] text-[#747b76]">Internal compliance workspace</p></div></div>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" className="hidden gap-2 text-[#59605b] sm:flex" onClick={() => setView("operations")}><ShieldCheck className="size-4" /> Admin area</Button>
      ...
      <main className="mx-auto max-w-6xl px-4 py-7 sm:px-8 sm:py-10">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_340px]">
          <section className="space-y-5">
            {showRound ? <RoundCard equipment={currentEquipment} index={roundIndex} total={activeEquipment.length} temperature={temperature} setTemperature={setTemperature} onSave={saveTemperature} onCancel={() => setShowRound(false)} failedReading={failedReading} actionSelections={actionSelections} setActionSelections={setActionSelections} actionNotes={actionNotes} setActionNotes={setActionNotes} onSaveAction={saveCorrectiveAction} /> : <TaskCard icon={<Thermometer className="size-5" />} eyebrow="Fridges" title="AM Temperatures" detail={dashboard?.tasks.find((task) => task.session === "AM")?.status === "complete" ? "Complete · all active units recorded" : "Due now · record each unit in order"} state={dashboard?.tasks.find((task) => task.session === "AM")?.status === "complete" ? "complete" : "due"} action={beginRound} actionLabel="Start check" />}
            <TaskCard icon={<Utensils className="size-5" />} eyebrow="Food probes" title="Cooking temperatures" detail={dashboard?.foodChecks.length ? `${dashboard.foodChecks.length} recorded today` : "Required when the first cooked batch is ready"} state={dashboard?.foodChecks.length ? "complete" : "upcoming"} action={() => setShowProbe(true)} actionLabel="Record probe" />
            <StatusCard icon={<ClipboardCheck className="size-5" />} eyebrow="Opening" title="Opening food-safety check" detail="Due when the store opens" state="Due" />
            <StatusCard icon={<Timer className="size-5" />} eyebrow="Closing" title="Closing food-safety check" detail="Due later today" state="Due later" />
...
function StatusCard({ icon, eyebrow, title, detail, state }: any) { return <div className="flex items-center gap-4 rounded-2xl border border-black/[0.07] bg-white p-5 sm:p-6"><div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-[#f6f7f5] text-[#89918b]">{icon}</div><div className="min-w-0 flex-1"><p className="text-[11px] font-semibold uppercase tracking-[0.15em] text-[#89918b]">{eyebrow}</p><h2 className="mt-1 text-lg font-semibold tracking-tight">{title}</h2><p className="mt-1 text-sm text-[#727a74]">{detail}</p></div><span className="text-xs font-semibold text-[#89918b]">{state}</span></div>; }
...
function RoundCard({ equipment, index, total, temperature, setTemperature, onSave, onCancel, failedReading, actionSelections, setActionSelections, actionNotes, setActionNotes, onSaveAction }: any) {
  const actionOptions = ["Rechecked temperature", "Checked fridge door", "Checked settings", "Moved food", "Removed food from sale", "Discarded affected food", "Manager informed", "Equipment taken out of use", "Maintenance reported"];
  if (failedReading) return <motion.div initial={{ opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl border border-[#efc8c3] bg-[#fff8f6] p-5 sm:p-7"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#b64738]">Action required</p><p className="mt-2 text-sm text-[#8d625d]">AM fridge checks · Unit {index + 1} of {total}</p></div><Button variant="ghost" size="sm" onClick={onCancel}><X className="mr-1 size-4" /> Close</Button></div><h2 className="mt-7 text-2xl font-semibold tracking-tight">{failedReading.name}</h2><p className="mt-2 text-4xl font-semibold tracking-tight text-[#b64738]">{failedReading.temperature}°C</p><p className="mt-2 text-sm text-[#8d625d]">Maximum permitted: 8°C. This reading requires corrective action.</p><p className="mt-7 text-sm font-semibold">What have you done?</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{actionOptions.map((option: string) => <button key={option} type="button" onClick={() => setActionSelections((current: string[]) => current.includes(option) ? current.filter((item) => item !== option) : [...current, option])} className={`cursor-pointer rounded-xl border px-3 py-3 text-left text-sm transition ${actionSelections.includes(option) ? "border-[#b64738] bg-[#b64738] text-white" : "border-black/[0.08] bg-white text-[#414943]"}`}>{actionSelections.includes(option) ? "✓ " : "□ "}{option}</button>)}</div><label className="mt-5 block text-sm font-semibold">Notes <textarea value={actionNotes} onChange={(event) => setActionNotes(event.target.value)} placeholder="Describe the problem, action taken and what happens next" className="mt-2 min-h-24 w-full rounded-xl border border-black/[0.1] bg-white p-3 text-sm outline-none ring-[#b64738] focus:ring-2" /></label><Button className="mt-4 h-12 w-full bg-[#202522] text-white" disabled={!actionNotes.trim()} onClick={onSaveAction}>Save action & continue <ChevronRight className="ml-1 size-4" /></Button></motion.div>;
  return <motion.div initial={{ opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl border border-[#d6c47a] bg-[#fffdf4] p-5 sm:p-7"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#997813]">AM fridge check</p><p className="mt-2 text-sm text-[#6e746f]">{index + 1} of {total}</p></div><Button variant="ghost" size="sm" onClick={onCancel}><X className="mr-1 size-4" /> Close</Button></div><div className="mt-7 flex items-center gap-3"><div className="flex size-12 items-center justify-center rounded-xl bg-[#f4c542] text-[#27251c]"><Thermometer className="size-6" /></div><div><h2 className="text-xl font-semibold tracking-tight">{equipment?.name ?? "Equipment"}</h2><p className="text-sm text-[#737a73]">{equipment?.type ?? "Temperature-controlled equipment"}</p></div></div><div className="mt-7 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-black/[0.07] bg-white px-4 py-3"><p className="text-xs text-[#7f877f]">Target</p><p className="mt-1 font-semibold">5°C or below</p></div><div className="rounded-xl border border-black/[0.07] bg-white px-4 py-3"><p className="text-xs text-[#7f877f]">Maximum</p><p className="mt-1 font-semibold">8°C</p></div></div><div className="mt-7"><label className="text-sm font-semibold">Temperature</label><div className="mt-2 flex gap-3"><Input autoFocus inputMode="decimal" type="number" step="0.1" value={temperature} onChange={(e) => setTemperature(e.target.value)} placeholder="4.2" className="h-16 bg-white text-2xl font-semibold" /><Button className="h-16 bg-[#202522] px-6 text-white" onClick={onSave} disabled={!temperature}>Save & next <ChevronRight className="ml-1 size-4" /></Button></div><p className="mt-2 text-xs text-[#858c85]">Enter the exact probe reading. Do not round.</p></div></motion.div>;
}
...
function AdminTile({ title, detail }: { title: string; detail: string }) { return <div className="rounded-xl border border-black/[0.07] bg-[#fafbf9] p-4"><div className="flex items-center justify-between"><p className="text-sm font-semibold">{title}</p><span className="text-xs font-semibold text-[#8b6c13]">Manage</span></div><p className="mt-2 text-xs text-[#89918b]">{detail}</p></div>; }
