...
function validateRequirement(title: string, nextDueAt: number) {
  if (!title.trim()) {
    throw new Error("Title is required");
  }
  if (!Number.isFinite(nextDueAt)) {
    throw new Error("Next due date must be valid");
  }
}
